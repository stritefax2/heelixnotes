...
export const getFullActivityText = async (activityId: number): Promise<string> => {
  const result = await invoke<[string, string] | null>('get_app_project_activity_text', { activityId });
  return result ? result[1] : "Document not found";
};

export const getFullActivityPlainText = async (activityId: number): Promise<string> => {
  const result = await invoke<[string, string] | null>('get_app_project_activity_plain_text', { activityId });
  return result ? result[1] : "Document not found";
};