...
// Get plain text version of activity for LLM consumption (no HTML formatting)
export const getActivityPlainText = async (activityId: number): Promise<string> => {
  const result = await invoke<[string, string]>('get_app_project_activity_plain_text', { activityId });
  return result[1]; // Returns [document_name, plain_text], we want the plain_text
};